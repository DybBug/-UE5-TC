...
#include "VectorWithNameSpinBox.generated.h"

class USpinBox;
class UTextBlock;

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnVectorChanged, const FVector&, _value);

/**
 * 
 */
UCLASS()
class TACTICALCOMBAT_API UVectorWithNameSpinBox : public UUserWidget
{
	GENERATED_BODY()
...
public:
	...
	FORCEINLINE const FVector& GetValue() const { return m_Value; }

	void SetValue(const FVector& _value);	
	
	FOnVectorChanged OnValueChanged;
...
private:
	...
	UFUNCTION()
	void _OnValueZChanged(float _z);
};
